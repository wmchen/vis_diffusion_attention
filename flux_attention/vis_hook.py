...
from mlcbase import Logger, EmojiProgressBar, create
...
class VisAttentionHook(AttentionHook):
    def __init__(
        self, 
        ...
        **kwargs
        ...
        super().__init__(flux_pipe, **kwargs)
        ...
        create(save_dir, "dir")
        self.logger = Logger()
        self.logger.init_logger(osp.join(save_dir, f"{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log"))
    ...
    def vis_attention_maps(
        ...
        assert self.cross_attn is not None or self.self_attn is not None, "inference the FLUX model before visualizing attention maps"
        if apply_heat_map:
        ...
        if self.vis_cross_attn:
            self.logger.info("visualizing cross attention maps...")
            ...
            self.logger.info("setting prompt...")
        ...
        if self.vis_self_attn:
            ...
            with EmojiProgressBar(total=len(selected_ids)) as pbar:
                for fig_id, index in enumerate(selected_ids):
                    ...
                    pbar.update(1)
            self.logger.info("saving self attention maps...")
            plt.savefig(osp.join(save_root, f"attention_maps{out_suffix}"), bbox_inches="tight")
            self.logger.success("done")
